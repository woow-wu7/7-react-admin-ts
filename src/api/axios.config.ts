const HOST =
  process.env.NODE_ENV === "development" ? "http://39.104.109.46:9090" : "";
...
export type TResuleWrap<D, R> = (
  body: D & { pageNum?: number; pageSize?: number; pageNo?: number }
) => Promise<{
  data: { data: R };
}>;

// 状态码判断
function getProcessStatus(status: number) {
  switch (status) {
    case 100:
    ...
    case 303:
      console.log(
        "sea other",
        "临时重定向，请求的页面可以在别的url找到, 必须使用get方法"
      );
      break;
    case 304:
      console.log(
        "not modified",
        "资源未被修改，缓存文档可以继续使用，主要用于协商缓存"
      );
      break;
    case 305:
